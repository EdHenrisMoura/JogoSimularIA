...
@export var speed = 200  # Velocidade de movimento do robô
...
func _process(_delta):
	if fase == 1:
		movimentar_automatico(_delta)
	elif fase == 2:
		movimentar_agrupar(_delta)
...
### Fase 1 ###
func movimentar_automatico(_delta):
	if move_to_objeto and alvo_atual:
		mover_para(alvo_atual, _delta)

### Fase 2: Agrupar ###
func movimentar_agrupar(_delta):
	if move_to_objeto and alvo_atual:
		mover_para(alvo_atual, _delta)
...
func processar_comando_agrupar(target):
	...
	elif alvos.has(target) and objetos_em_mao.size() > 0:
		largar_objeto(target)
		if verificar_conclusao():
			finalizar_fase_2()
...
func verificar_conclusao():
	for alvo in alvos:
		if alvo.get_meta("ocupado") == false:
			return false
	return true
...
func _on_executar_pressed():
	...
	if comando_sequencia == ["andar", "bola"]:
		...
		if alvo_atual:
			print("Indo para a bola mais próxima:", alvo_atual)
		else:
			print("Nenhuma bola disponível.")
	elif comando_sequencia == ["andar", "circulo"]:
		move_to_objeto = true
		alvo_atual = obter_alvo_mais_proximo()
		if alvo_atual:
			print("Indo para o alvo mais próximo:", alvo_atual)
		else:
